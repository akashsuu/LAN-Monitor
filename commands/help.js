...
const helpCommand = {
  execute() {
    ...
    output.push('    ln find <keyword>    Search devices by IP, MAC, vendor, etc.');
    output.push('');
    output.push(chalk.bold('  NETWORK COMMANDS'));
    output.push(chalk.gray('  \u2500'.repeat(20)));
    output.push('    ln ping <host>       Ping a host (--stats for extended)');
    output.push('    ln latency <host>    Detailed latency test with jitter');
    ...
    output.push('    ln info <ip>         Detailed device info (OS, MAC type)');
    output.push('    ln os <ip>           Detect operating system');
    ...
    output.push('    ln port scan <ip>    Advanced port scanner');
    output.push('');
    ...
    output.push('    ln traffic           Monitor live network traffic');
    output.push('');
    output.push(chalk.bold('  MANAGEMENT'));
    output.push(chalk.gray('  \u2500'.repeat(20)));
    output.push('    ln dashboard [--live] Start the dashboard (live mode)');
    output.push('    ln alerts            Manage alerts');
    output.push('    ln report [period]   Generate report (today|week|month|html)');
